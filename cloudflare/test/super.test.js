...
// 두 곳이 서로 다른 말을 하고 있었다. 위(시크릿 옮기기)는 "현행 값을 그대로 옮기세요",
// 아래(전자서명 보안)는 "키를 새로 만들거나…" — 아래를 따르면 이미 받은 서명이 전부 깨진다.
// 되돌릴 수 없는 조작이라, 화면 어디에도 그 길을 열어 두지 않는다.
test("보안 안내가 '키를 새로 만드는 길'을 권하지 않는다", async () => {
  const html = await superHtml();
  assert.match(html, /서명 개인키가 데이터베이스에 저장되어 있습니다/, "경고 자체는 떠야");
  assert.ok(!/키를 새로 만들거나/.test(html), "새 키 생성을 절차로 안내하면 안 된다");
  assert.ok(!/gen-sign-key/.test(html), "생성 스크립트를 화면에서 권하면 안 된다");
  assert.ok(!/DELETE FROM settings/.test(html), "손으로 DB 를 지우게 하지 않는다 (버튼이 있다)");
  assert.match(html, /href="#s-secret"/, "위의 옮기기 절차로 보내야");
  assert.match(html, /id="s-secret"/, "그 절차에 갈 자리가 있어야");
});
