...
const CAT_RULES = [
  ["음식점", ["음식", "식당", "한식", "중식", "일식", "양식", "분식", "고깃집", "정육식당", "치킨", "피자", "국밥", "칼국수", "횟집", "주점", "호프", "술집", "포차", "이자카야", "바(bar)", "요리",
    // 상호에서 짐작할 때 걸리는 낱말들 — 명부에 업종 칸이 없던 실제 사례(초밥·양꼬치·식탁)에서 나왔다
    "초밥", "참치", "양꼬치", "꼬치", "식탁", "돈까스", "돈가스", "삼겹", "갈비", "곱창", "족발", "보쌈", "냉면", "만두", "쭈꾸미", "낙지", "감자탕", "찌개", "탕", "짬뽕", "짜장", "마라", "쌀국수", "우동", "라멘", "파스타", "버거", "샌드위치", "김밥", "떡볶이", "치킨"]],
  ...
  ["생활·서비스", ["노래", "부동산", "공인중개", "미용", "뷰티", "네일", "피부", "이용원", "이발", "세탁", "수선", "철물", "열쇠", "자동차", "카센터", "정비", "세차", "사진", "인화", "동물병원", "약국", "의원", "병원", "한의", "치과", "침", "뜸", "마사지", "안마", "인테리어", "설비", "통신", "휴대폰", "편의점", "마트", "슈퍼", "숙박", "호텔", "모텔", "여관", "고시원", "세무", "법무", "회계", "노무", "제조", "유통", "물류", "택배", "인쇄", "복사", "은행", "보험", "카센타", "pc방", "피시방", "오락",
    "헤어", "살롱", "뷰티샵", "왁싱", "속눈썹", "타투", "동물", "펫", "애견", "종합기획", "기획", "광고", "디자인", "스튜디오"]],
];
...
export function parseMemberRoster(text, { prefix = "" } = {}) {
  ...
  for (let r = 0; r < body.length; r++) {
    ...
    // 업종 칸이 비었으면 **상호로 짐작**합니다. "엄마손 칼국수" 가 음식점인 줄 모르는 사람은 없는데,
    // 그걸 '기타' 로 넣으면 손님 화면의 분류 단추에서 그 가게가 빠집니다.
    // 짐작이라는 것을 표에 적어 둡니다 — 조용히 넘겨짚지 않습니다. 못 맞히면 '기타' 가 맞습니다.
    const guessed = !rawCat && mapCategory(name) !== "기타" ? mapCategory(name) : "";
    const row = {
      seq: r + 1, name, owner, phone, phoneFixed: fixed, address,
      rawCat, category: rawCat ? mapCategory(rawCat) : (guessed || "기타"), catGuessed: !!guessed,
      status: "ok", note: "",
    };
