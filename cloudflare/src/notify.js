...
본 안내는 위 계약서의 서명 당사자로 지정되신 분께만 발송됩니다.`,
  ...
  // ⚠️ 카카오 반려 이력 (2026-08-26 · 08-31)
  //   "'전자서명' 은 광범위한 말머리" → 무엇에 대한 서명인지가 **고정값**으로 드러나야 한다.
  //   심사자가 직접 준 수정 예시가 '전자계약서 서명 미완료' 였다. 그 말을 그대로 쓴다.
  //   그리고 말머리를 #{상호} 뒤에 두면 심사자는 변수 뒤의 글자만 제목으로 읽는다 —
  //   그래서 고정 제목을 **맨 앞**에 두고 상호는 본문 안으로 넣었다.
  sign_remind: {
    key: "tpl_sign_remind", label: "전자계약서 서명 미완료 안내", button: "전자계약서 서명하기",
    vars: ["상호", "이름", "문서명", "기한"],
    body: `전자계약서 서명 미완료 안내

#{이름}님, #{상호}에서 요청드린 전자계약서에 아직 서명이 완료되지 않아 다시 안내드립니다.

▶ 계약서명: #{문서명}
▶ 서명 기한: #{기한}
▶ 요청 기관: #{상호}

기한이 지나면 서명 링크가 만료되어 더 이상 서명하실 수 없습니다.
아래 버튼을 눌러 계약 내용을 확인하신 후 전자서명을 완료해 주세요.

본 메시지는 #{상호}로부터 전자계약서 서명을 요청받으신 분 중 아직 서명을 완료하지 않으신 분께만 발송되는 안내이며, 이미 서명을 마치신 분께는 발송되지 않습니다.`,
  },
  sign_done: {
...
문의: #{상호}`,
  ...
  // ⚠️ 아직 제품에서 **한 번도 발송되지 않는** 문구다. 공지를 알림톡으로 보내는 기능이 없다.
  //   카카오도 2026-08-31 에 반려했다 — "'공지사항' 은 광범위한 타이틀", "내용이 변수뿐이라
  //   발송 목적을 판단할 수 없음", "목적별로 템플릿을 나눠 등록하라".
  //   맞는 지적이다. 회비 안내·총회 안내·투표 안내는 서로 다른 메시지이고, 그 기능을 만들 때
  //   목적마다 따로 등록해야 한다. **그때까지는 카카오에 올리지 않는다** —
  //   없는 기능의 심사를 반복해서 받을 이유가 없다.
  // 회비 독촉. 위 notice 반려 사유("'공지사항' 은 광범위한 타이틀 · 목적별로 나눠 등록하라")를
  // 그대로 반영해 **목적이 제목에 고정값으로 드러나는** 별도 템플릿으로 만든다.
  // 금액·계좌를 본문에 넣어야 받는 사람이 총무에게 되묻지 않는다.
  //
  // ⚠️ pending — 아직 카카오에 올리지 않았다. 코드는 완성돼 있고 템플릿 코드가 등록되는 순간
  //   동작한다. 등록 전에는 발송이 시도되지 않고 화면이 그 사실을 그대로 말한다.
  dues_remind: {
    key: "tpl_dues_remind", label: "회비 납부 안내", button: "회비 내역 확인하기", pending: true,
    vars: ["상호", "이름", "납부월", "금액", "계좌"],
    body: `회비 납부 안내

#{이름}님, #{상호}의 #{납부월} 회비 납부가 확인되지 않아 안내드립니다.
...
입금하신 뒤 총무에게 알려 주시면 장부에 반영해 드립니다.
이미 납부하셨다면 이 안내는 무시하셔도 됩니다.
본 안내는 #{상호} 회원 중 해당 월 회비 납부가 확인되지 않은 분께만 발송됩니다.`,
  },
  // 사장님께 사진·영업시간 부탁. 회장님이 123명에게 카톡을 손으로 붙여 보내는 것을 대신한다.
  // 목적이 제목에 고정값으로 드러나고(반려 사유 반영), 링크는 사람마다 다르므로 버튼이 아니라
  // 본문 변수로 싣는다 — 알림톡 버튼 주소는 템플릿에 고정돼야 해서 사람마다 바꿀 수 없다.
  //
  // ⚠️ pending — 아직 카카오에 올리지 않았다. 템플릿 코드가 등록되는 순간 동작한다.
  photo_ask: {
    key: "tpl_photo_ask", label: "가게 사진·영업시간 요청", button: "", pending: true,
    vars: ["상호", "이름", "가게", "링크"],
    body: `가게 사진·영업시간 요청

#{이름}님, #{상호} 홈페이지에 올릴 #{가게}의 사진과 영업시간을 부탁드립니다.

▶ 단체명: #{상호}
▶ 가게: #{가게}
▶ 올리는 곳: #{링크}

위 링크를 누르시면 로그인 없이 휴대폰에서 바로 올리실 수 있습니다. (2주 안에 열어 주세요)
본 안내는 #{상호} 회원 중 사진 또는 영업시간이 아직 없는 가게에만 발송됩니다.`,
  },
  notice: {
    key: "tpl_notice", label: "회원 공지사항 안내", button: "공지 내용 확인하기", pending: true,
    vars: ["상호", "이름", "제목", "내용"],
    body: `[#{상호}] 회원 공지사항 안내
