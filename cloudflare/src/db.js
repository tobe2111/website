...
export function updateBusiness(db, id, f) {
  return run(db, "UPDATE businesses SET name=?, category=?, description=?, phone=?, address=?, hours=?, lat=?, lng=?, sns_instagram=?, sns_youtube=?, sns_blog=?, sns_kakao=?, sns_naver=?, map_url=?, updated_at=datetime('now') WHERE id=?",
    f.name, f.category, f.description, f.phone, f.address, f.hours, f.lat ?? null, f.lng ?? null,
    f.snsInstagram || "", f.snsYoutube || "", f.snsBlog || "", f.snsKakao || "", f.snsNaver || "", f.mapUrl || "", id);
}
