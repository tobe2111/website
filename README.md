...
- **조직 유형(kind)**: 같은 엔진 위에서 여러 제품이 갈립니다 — `merchant`(상인회 홈페이지) ·
  `esign`(전자계약 전용) · `franchise`(**모집 랜딩 + 상담 DB**). 제품 소개 페이지는 각각
  `/`, `/esign`, `/homepage`.
- **업종 프리셋**: 모집형은 문구만 갈아 끼워 업종을 바꿉니다 — 프랜차이즈 · 학원 · 헬스장 ·
  병원 · 분양. 정의는 `cloudflare/src/kinds.js` 한 곳에 있고, 추가 방법은
