...
> ⚠️ **운영 버전은 [`cloudflare/`](./cloudflare/) 입니다.** 루트의 Node.js 버전(`src/`)은
> 초기 레퍼런스 구현으로, 이후 기능(제품 진열·플랫폼 입점·플랜·이메일·디자인 시스템 v2)은
> **Cloudflare 버전에만** 반영됩니다. 새 기능 개발·배포는 `cloudflare/` 에서만 진행하세요.
