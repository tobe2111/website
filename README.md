...
> **코드는 [`cloudflare/`](./cloudflare/) 가 전부입니다.** (초기 Node.js 레퍼런스 구현은
> 정리·제거되었습니다 — git 이력에서 열람 가능.) 개발·테스트·배포 모두 `cloudflare/` 에서 진행하세요.
...
- **조직 유형(kind)**: 같은 엔진 위에서 세 가지 제품이 갈립니다 — `merchant`(상인회 홈페이지) ·
  `esign`(전자계약 전용) · `franchise`(**프랜차이즈 가맹점 모집 랜딩 + 상담 DB**).
  제품 소개 페이지는 각각 `/`, `/esign`, `/homepage` 입니다. 자세한 내용은
  [`cloudflare/README.md`](./cloudflare/README.md#조직-유형kind--같은-엔진-세-가지-제품).
...
- 🏁 **프랜차이즈 가맹점 모집 랜딩** — 한 장짜리 모집 페이지(강점·후기·절차·비용·상담 폼) + **상담 신청 DB**(상태·메모·CSV·유입 경로 집계)
