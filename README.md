...
- **통합 테스트**: `npm test` — 임시 DB로 서버를 띄워 HTTP로 검증(홈·테넌트·404·보안헤더·CSRF·로그인/역할·권한·테넌트 격리·JSON-LD·검색·게시판·**영상 임베드/CSP**·점포 지도·sitemap/robots·헬스체크·비밀번호 재설정·**전자서명 생성/서명/검증**). 22개, 외부 의존성 없음.
