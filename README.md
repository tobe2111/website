...
## 미디어 업로드 & 뷰어 (사진 / 세로영상·릴스)

- 업체 대시보드에서 사진·영상을 **드래그앤드롭 또는 파일 선택**으로 여러 개 한 번에 올립니다. 별도 변환 없이 바로 게시됩니다.
- 영상은 **HTTP Range 스트리밍**으로 제공되어 재생 중 탐색(seeking)이 됩니다.
- 업체 페이지 **및 대시보드 미리보기**에서 사진·영상을 클릭하면 **전체화면 뷰어(라이트박스)**가 열립니다:
  - **세로 영상(인스타 릴스·쇼츠형)도 잘리지 않고 9:16 비율 그대로** 표시 — 가로/세로를 자동 판별.
  - 좌우 화살표·키보드(←/→/Esc)·모바일 스와이프로 넘기기, 영상 자동재생·컨트롤 제공.
- 갤러리 썸네일에는 영상 배지(🎬)와 재생(▶) 아이콘, 그리고 (아래 파이프라인이 만든) **포스터 이미지**가 표시됩니다.

### 미디어 처리 파이프라인 (ffmpeg 자동 감지)

업로드 시 `src/media.js` 가 실행됩니다. **`ffmpeg` 시스템 바이너리가 있으면 자동으로**:

- 영상을 **웹 표준 H.264 mp4**(+`faststart`)로 정규화 → 브라우저 호환성 극대화 (HEVC/.mov 문제 해소)
- 영상 **포스터(썸네일) 프레임**을 추출해 갤러리·뷰어에 사용
...
`ffmpeg` 가 없으면 **원본을 그대로 통과**시킵니다(안전한 폴백). 즉 실제 서버에 `ffmpeg` 만 설치하면 **코드 변경 없이** 트랜스코딩·영상 포스터·이미지 썸네일이 켜집니다. 서버 시작 로그에 현재 상태(`미디어: ffmpeg 있음/없음`)가 표시됩니다.
...
- **이미지 첨부**: 글마다 사진 1장 첨부(매직바이트 검증·최대 8MB). 목록에 썸네일·📎 표시, 상세에 본문 이미지 노출.
...
- **통합 테스트**: `npm test` — 임시 DB로 서버를 띄워 HTTP로 검증(홈·테넌트·404·보안헤더·CSRF·로그인/역할·권한·테넌트 격리·JSON-LD·검색·sitemap/robots·헬스체크·비밀번호 재설정·**전자서명 생성/서명/검증**). 16개, 외부 의존성 없음.
