...
- 업체 페이지의 사진·영상을 클릭하면 **전체화면 뷰어(라이트박스)**가 열립니다:
  - **세로 영상(인스타 릴스·쇼츠형)도 잘리지 않고 9:16 비율 그대로** 표시 — 가로/세로를 자동 판별.
  - 좌우 화살표·키보드(←/→/Esc)·모바일 스와이프로 넘기기, 영상 자동재생·컨트롤 제공.
- 갤러리 썸네일에는 영상 배지(🎬)와 재생(▶) 아이콘이 표시됩니다.

> **포맷 안내(정직한 한계)**: 서버측 트랜스코딩/썸네일 생성 라이브러리(ffmpeg·sharp)는 이 환경의 패키지 설치 제한으로 사용하지 않습니다. 따라서 업로드한 **원본이 그대로** 제공됩니다. 대부분의 브라우저에서 재생되는 **H.264 mp4**(대부분의 스마트폰 촬영·릴스 저장 포맷) 사용을 권장하며, 일부 HEVC(.mov) 코덱은 브라우저에 따라 재생되지 않을 수 있습니다. 운영 규모가 커지면 업로드 시 mp4(H.264)로 변환하는 별도 처리(예: 외부 미디어 서비스)를 붙이는 것을 권장합니다.
