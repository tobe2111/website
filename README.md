...
`ffmpeg` 가 없으면 **원본을 그대로 통과**시킵니다(안전한 폴백). 즉 실제 서버에 `ffmpeg` 만 설치하면 **코드 변경 없이** 트랜스코딩·썸네일이 켜집니다. 서버 시작 로그에 현재 상태(`미디어: ffmpeg 있음/없음`)가 표시됩니다.
...
> 남은 최적화(외부/대규모): 이미지 썸네일·리사이즈(ffmpeg/sharp), 업로드 완전 스트리밍 파서, CDN, 정적 자산 fingerprint 불변 캐시.
